...
| profile | 二维码 | 初始页面 | 页面范围 | 截图等待范围 | 超时 | 理论峰值 |
|---|---:|---:|---:|---:|---:|---:|
| `safe` | 512px | 1000B | 512–1600B | 200–1200ms | 2500ms | 约 8KB/s |
| `balanced`（默认） | 512px | 1600B | 512–2100B | 60–600ms | 1200ms | 约 35KB/s |
| `fast` | 640px | 1800B | 384–2150B | 30–350ms | 800ms | 约 72KB/s |
...
显式参数优先于 profile 预设。吞吐量主要由“实际每页有效字节 ÷ 每页确认周期”决定；表中峰值只按最大页面和最短等待计算，实际速度还会受截图、解码、按键确认、远程桌面刷新和重传影响。二维码过密或等待过短都会触发降速，不能只追求单个参数的最大值。大屏幕可在发送端增加 `--qr-size 768` 改善高密度二维码的识别稳定性。
