...
| `fast` | 640px | 1800B | 384–2150B | 0ms | 连续 3 次 800ms |
...
显式参数优先于 profile 预设。Balanced/Fast 的 `0ms` 表示程序不额外休眠，实际确认周期仍由截图、解码、按键和远程桌面刷新决定；因此不提供脱离实际环境的理论峰值。大屏幕可在发送端增加 `--qr-size 768` 改善高密度二维码的识别稳定性。
